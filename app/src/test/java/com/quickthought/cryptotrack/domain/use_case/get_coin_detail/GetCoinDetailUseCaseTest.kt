...
class GetCoinDetailUseCaseTest {
    ...
    @Test
    fun `UseCase should emit Loading then Success when both repository calls succeed`() =
        runTest {
            val fakeCoin = Coin("btc", "Bitcoin", "BTC", 50000.0, 1.0, "")
            val fakeChart = ChartData(listOf(49000f, 50000f))

            // Stubbing both repository calls
            coEvery { repository.getCoinById("btc") } returns
                flow {
                    emit(Resource.Loading())
                    emit(Resource.Success(fakeCoin))
                }
            coEvery { repository.getCoinChart("btc") } returns
                flow {
                    emit(Resource.Loading())
